...
class PatentApp:
    ...
    def setup_ui(self):
        ...
        style.configure("Treeview.Heading", background=self.colors['card'], foreground=self.colors['text_light'], relief="flat", font=self.fonts['bold'])
        style.configure("Treeview", background=self.colors['card'], fieldbackground=self.colors['card'], foreground=self.colors['text'], rowheight=28, borderwidth=0, font=self.fonts['main'])
        style.map("Treeview", background=[('selected', self.colors['accent'])], foreground=[('selected', '#FFFFFF')])
        style.configure("Vertical.TScrollbar", gripcount=0, background=self.colors['bg'], troughcolor=self.colors['bg'], bordercolor=self.colors['bg'], arrowcolor=self.colors['text_light'])
        ...
        self.url_entry = tk.Entry(top_frame, width=50, font=self.fonts['main'], relief='flat', highlightthickness=1, highlightbackground=self.colors['border'])
        ...
        def create_filter_btn(val, key):
            btn = tk.Radiobutton(filter_frame, text=key, variable=self.filter_var, value=val,
                                 indicatoron=0, command=self.refresh_right_view,
                                 bg=self.colors['card'], selectcolor=self.colors['active_filter'],
                                 fg=self.colors['text'], font=self.fonts['main'], relief='flat', bd=0, padx=10, pady=3,
                                 cursor='hand2')
    ...
    def refresh_local_list(self):
        ...
        for d in dirs:
            ...
            # iid=pid (用于内部索引), values=(Title, Date, Pending)
